...
namespace CopticDictionarynew1.Controllers
{
        ...
        public async Task<IActionResult> Index(string? search, string? searchType = "exact")
        {
            ...
            switch (searchType)
            {
                ...
                case "contain":
                    ...
                    wordsList = wordsList.Where(w => NormalizeString(w.Word_text).Contains(search)).ToList(); break;

                case "start":
                    // Starts with search term
                    wordsList = wordsList.Where(w => NormalizeString(w.Word_text).StartsWith(search)).ToList(); break;


                case "end":
                    // Ends with search term
                    wordsList = wordsList.Where(w => NormalizeString(w.Word_text).EndsWith(search)).ToList(); break;
                    break;
    ...
    new SelectListItem { Value = "ⲏⲡⲓ", Text = "Number - عدد" }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateDerivedWord(int WordID, [Bind("Word_text,Language,Class,notes,IPA,Pronunciation,IsDrevWord,IsReviewed,RootID,GroupID")] Word word)
        ...
        public IActionResult SelectFromDerivedWord(int WordId1)
        {
            // Ensure WordId1 exists
            var word1 = _context.Words.Find(WordId1);
            if (word1 == null)
            {
                return NotFound("The original word does not exist.");
            }

            // Pass the list of existing words to the view, excluding WordId1
            ...
            ViewBag.WordId1 = WordId1; // Pass WordId1 to the view
            return View();
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SelectFromDerivedWord(int WordId1, int RelatedWordID)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // Update RelatedWord's IsDrevWord field
                word2.IsDrevWord = true;
                _context.Words.Update(word2);
                await _context.SaveChangesAsync();
